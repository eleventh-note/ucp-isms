...
//::END OF 'CONFIGURATION'
	...
	$school_years = $hnd->GetSchoolYears();
			...
			<div id="body">			
				...
				<div class="content">
					...
					<div class="column" id="column-second">
						<div class="inner">
							<h1>
								<span class="Highlight">School Administration &raquo; School Years</span> 
							</h1>
							<div id="actions">
							...
							<p class="">The list of school years below are used to define curriculums and the I-SMS current school year settings.</p>
							<?php
								//##### PASS ERROR IF FOUND
								if(isset($success)){
									...
									<?php 
										...
										foreach($school_years as $item){
											...
											if($ctr % 2 == 0){
												...
												echo "<td>{$ctr}</td>";
												echo "<td>{$item->start} - {$item->end}</td>";
												if ($item->active == 1){
													echo "<td><b>YES</b></td>";
												} else { echo "<td>NO</td>"; }
												echo "<td class=\"Actions\">";
												if($item->active == 0){
													echo "<a href=\"schoolplusplus-school_years-process.php?id={$item->year_id}&action=activate\">Activate</a>";
													echo " | ";
												}												
													echo "<a href=\"schoolplusplus-school_years-process.php?id={$item->year_id}&action=delete\" onclick=\"return confirm('Delete School Year? Click OK to continue.')\">Delete</a>";
												echo "</td>";
											echo "</tr>";
										}
									?>
							...
							<form action="schoolplusplus-school_years-process.php" method="post">
								<hr class="form_top"/>
								<div class="table_form">
									<table class="form" cellspacing="0">
										<tr>
											<a name="SchoolYears"></a>
											<td class="label">Starting Year [yyyy]</td>
											<td class="input">: <input type="text" name="start_year" class="input_position" /> <input type="submit" class="button" name="year_save" value="Save School Year" /></td>
										</tr>
									</table>						
									<hr class="form_top"/>
								</div>
