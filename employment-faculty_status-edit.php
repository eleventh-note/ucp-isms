...
//::END OF 'CONFIGURATION'
	...
	if(isset($_GET['id'])){
		...
		//get statuss array
		$statuses = $hnd->GetFacultyStatuses($id);
		$status = $statuses[0];		
	} else {
		$_SESSION['error'] = array("Faculty Status not found.");
			...
			<div id="body">			
				...
				<div class="content">
					...
					<div class="column" id="column-second">
						<div class="inner">
							<h1>
								<span class="Highlight">Faculty Administration &raquo; Manage Faculty Status &raquo; Edit</span> 
							</h1>
							<p class="">Input new description for faculty status '<b><?php echo $status->description; ?></b>' below.</p>
							<?php
								//##### PASS ERROR IF FOUND
								if(isset($error)){
							...
							?>
							<form action="employment-faculty_status-process.php" method="post">
								<?php echo "<input type=\"hidden\" name=\"id\" value=\"{$status->status_id}\" />"; ?>
								<hr class="form_top"/>
								<div class="table_form">
									<table class="form" cellspacing="0">
										<tr>
											<td class="label">New Faculty Status Description</td>
											<td class="input">: <input type="text" name="status" class="input_status" /></td>
										</tr>
									</table>						
									<hr class="form_top"/>
									<table class="form" cellspacing="0">
										<tr class="button">
											<td colspan="2">
												<input type="button" class="button" value="Cancel" onclick="window.location='employment-faculty_status.php';"/>
												<input type="submit" class="button" name="status_save" value="Save" />
											</td>
										</tr>
									</table>
								</div>
