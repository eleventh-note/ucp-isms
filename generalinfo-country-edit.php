...
//::END OF 'CONFIGURATION'
	...
	if(isset($_GET['id'])){
		$id = (int) $_GET['id'];
		$countries = $hnd->GetCountries($id);
		
		if(sizeof($countries) == 0){
			$_SESSION['error'] = array("Unknown country.");
			header("Location: generalinfo-country.php");
			exit();		
		}
	} else {
		$_SESSION['error'] = array("Unknown country..");
			...
			<div id="body">			
				...
				<div class="content">
					...
					<div class="column" id="column-second">
						<div class="inner">
							<h1>
								<span class="Highlight">General Information &raquo; Country Listing &raquo; Edit Country</span> 
							</h1>
							<p class="">Input new details for '<b><?php echo $countries[0]->description; ?></b>' below.</p>
							<?php
								//##### PASS ERROR IF FOUND
								if(isset($error)){
							...
							?>
							<form action="generalinfo-country-process.php" method="post">
								<input type="hidden" name="id" value="<?php echo $countries[0]->country_id; ?>" />
								<hr class="form_top"/>
								<div class="table_form">
									<table class="form" cellspacing="0">
										...
										<tr>
											<td class="label">Country Acronym</td>
											<td class="input">: <input type="text" name="code" class="input_code" /></td>
										</tr>
									</table>						
									<hr class="form_top"/>
									<table class="form" cellspacing="0">
										<tr class="button">
											<td colspan="2">
												<input type="button" class="button" value="Cancel" onclick="window.location='generalinfo-country.php';"/>
												<input type="submit" class="button" name="country_save" value="Save Information" />
											</td>
										</tr>
									</table>
								</div>
