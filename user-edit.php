...
//::END OF 'CONFIGURATION'
	...
	$user = $hnd->GetUsers(null,null, $id);	
	$employees = $emp->GetEmployeesWithoutUser($user[0]->employee_id);
	...
	$employee = $user[0]->employee_id;
	$priv = array();
	
	if(sizeof($user[0]->privileges) > 0){
		foreach($user[0]->privileges as $p){
			$priv[] = $p->id;
		}
	}
			...
			<div id="body">			
				...
				<div class="content">
					...
					<div class="column" id="column-second">
						<div class="inner">
							<h1>
								<span class="Highlight">User Administration &raquo; Edit User</span> 
							</h1>
							<p class="">Input user details. Password will be changed when editing user details.</p>
							<?php
								//##### PASS ERROR IF FOUND
								if(isset($error)){
								...
								<div class="table_form">
									<table class="form" cellspacing="0">
										<tr>
											...
											<td class="input">: <input type="text" name="username" class="input_description" value="<?php echo $username; ?>" readonly="readonly"/></td>
										</tr>
										<tr>
													...
													<?php
														foreach($employees as $item){
															if($employee == $item->employee_id){
																...
																echo "<option disabled=\"disabled\" value=\"{$item->employee_id}\">{$item->last_name}, {$item->first_name} {$item->middle_name}</option>";
															}
														}
													?>
												</select>
									...
									<table class="form" cellspacing="0">
										<tr class="button">
											<td colspan="2">
												...
												<input type="submit" class="button" name="add_user" value="Save User" />
											</td>
										</tr>
									</table>
								</div>
