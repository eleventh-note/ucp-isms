...
//::END OF 'CONFIGURATION'
	...
	if(isset($_GET['cid']) && isset($_GET['cud'])){
		...
		if(sizeof($colleges) > 0 && sizeof($courses) > 0){
			...
			$sys = $hnd_sc->GetSchoolYears();

			//redirect if no courses found
			if(sizeof($sys) == 0){
				$_SESSION['error'] = array("There are no school years defined.");
				header("Location: schoolplusplus-curriculums-select-course.php?cid={$college_id}");
				exit();
			}
			...
			<div id="body">			
				...
				<div class="content">
					...
					<div class="column" id="column-second">
						<div class="inner">
							<h1>
								<span class="Highlight">Curriculum Administration &raquo; Curriculum &raquo; Select School Year</span>
							</h1>
							...
							<form action="colleges-process.php" method="post">
								...
								<div class="table_form">
									...
									<table class="form" cellspacing="0">
											<td>
												<input type="button" class="button" value="Back" onclick="window.location='schoolplusplus-curriculums-select-course.php?cid=<?php echo $college_id; ?>';"/>
												<input type="button" class="button"  value="Next" onclick="if(getSelectValue('oSchoolYear') > 0){
												
												redirectTo('schoolplusplus-curriculums-select-semyear.php?cid=<?php echo $college_id; ?>&cud=<?php echo $course_id; ?>&sy=', getSelectValue('oSchoolYear')); } else { alert('Please select a school year') }" />
