	...
	if(isset($_POST['scholarship_save'])){
		...
		switch($action){
			case 'Add':
				...
				if($hnd->AddDiscount($description, $price, $percentage) == true){
					
					//modification - 20130706 - Audit Trail
					$audit->action = "Add Discount";
					...
					$_SESSION['success'] = "Discount successfully added!";
					header("Location: finance-discount.php");
					exit();
				} else {
					$_SESSION['error'] = $hnd->error;
					header("Location: finance-discount-add.php");
					exit();
				}
				break;
			case 'Save':
				...
				if($hnd->EditDiscount($id, $description, $price, $percentage) == true){
				
					//modification - 20130706 - Audit Trail
					$audit->action = "Update Discount";
					$audit->newValue = str_replace("{0}", $description, $audit->newValue);
					$audit->newValue = str_replace("{1}", $price, $audit->newValue);
					$audit->newValue = str_replace("{2}", $percentage, $audit->newValue);
					$auditor->Add($audit);
					...
					$_SESSION['error'] = $hnd->error;
					header("Location: finance-discount-edit.php?id=$id");
					exit();
				}
				break;
		}//end switch
		...
		$hnd = new FinanceManager($conn);
		$id = (int) $_GET['id'];
		if($hnd->DeleteDiscount($id) == true){
			
			//modification - 20130706 - Audit Trail
			$audit->action = "Delete Discount";
			$audit->newValue = "DiscountId: {0}";
			$audit->newValue = str_replace("{0}", $id, $audit->newValue);
			$auditor->Add($audit);
			
			$_SESSION['success'] = "Discount successfully deleted!";
			header("Location: finance-discount.php");
			exit();
		} else {
			$_SESSION['error'] = $hnd->error;
		...
		header("Location: finance-discount.php.php");
		exit();
	}
