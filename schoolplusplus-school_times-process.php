	...
	if(isset($_POST['time_save'])){
		...
		switch($action){
			case 'Save Time':
				...
				if($hnd->AddSchoolTime($time) == true){;
					$_SESSION['success'] = "School Time successfully added!";
					header("Location: schoolplusplus-school_times.php");
					exit();
				} else {
		...
		$hnd = new SchoolManager($conn);
		$id = (int) $_GET['id'];
		if($hnd->DeleteSchoolTime($id) == true){
			$_SESSION['success'] = "School Time successfully deleted!";
			header("Location: schoolplusplus-school_times.php");
			exit();
		} else {
			$_SESSION['error'] = $hnd->error;
