			...
			<div id="body">			
				...
				<div class="content">
					...
					<div class="column" id="column-second">
						<div class="inner">
							<h1>
								<span class="Highlight">Employee Administration &raquo; Manage Employees</span> 
							</h1>
							<p class="">Below are the employees registered in the I-SMS.</p>
							<div id="actions">
								<p class="action">
									<input type="button" value="Go Back" onclick="window.location='employment.php'" />
									<input type="button" value="Add Employee" onclick="window.location='employment-employee-add.php'" />
								</p>
							</div>
							<?php
								//##### PASS ERROR IF FOUND
							...
							<div class="table">
								<table class="employees" cellspacing="0" title="">
									<thead>
										...
										<th class="date_of_entry">Date of Entry</th>
										<th class="employee_status">Employee Status</th>
									...
									<?php 
										...
										if(sizeof($employees) > 0){
											foreach($employees as $item){
												...
												if($ctr % 2 == 0){
													echo "<tr class=\"even\" onclick=\"window.location='employment-employee-view.php?id={$item->employee_id}';\">";
												} else {
													echo "<tr class=\"odd\" onclick=\"window.location='employment-employee-view.php?id={$item->employee_id}';\">";
												}
													echo "<td>{$ctr}</td>";
													echo "<td>{$item->last_name}, {$item->first_name} " . substr($item->middle_name,0,1) . ".</td>";
													echo "<td>{$item->employee_number}</td>";
													echo "<td>{$item->date_of_entry}</td>";
													echo "<td>{$item->employee_status}</td>";
											...
											echo "<td colspan=\"6\">There are no existing employee records.</td>";
										}
									?>
