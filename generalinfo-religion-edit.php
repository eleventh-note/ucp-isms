...
//::END OF 'CONFIGURATION'
	...
	if(isset($_GET['id'])){
		$id = (int) $_GET['id'];
		$religions = $hnd->GetReligions($id);
		
		if(sizeof($religions) == 0){
			$_SESSION['error'] = array("Unknown religion.");
			header("Location: generalinfo-religion.php");
			exit();		
		}
	} else {
		$_SESSION['error'] = array("Unknown religion	.");
			...
			<div id="body">			
				...
				<div class="content">
					...
					<div class="column" id="column-second">
						<div class="inner">
							<h1>
								<span class="Highlight">General Information &raquo; Religion Listing &raquo; Edit Religion</span> 
							</h1>
							<p class="">Input new details for '<b><?php echo $religions[0]->description; ?></b>' below.</p>
							<?php
								//##### PASS ERROR IF FOUND
								if(isset($error)){
							...
							?>
							<form action="generalinfo-religion-process.php" method="post">
								<input type="hidden" name="id" value="<?php echo $religions[0]->religion_id; ?>" />
								<hr class="form_top"/>
								<div class="table_form">
									...
									<table class="form" cellspacing="0">
										<tr class="button">
											<td colspan="2">
												<input type="button" class="button" value="Cancel" onclick="window.location='generalinfo-religion.php';"/>
												<input type="submit" class="button" name="religion_save" value="Save Information" />
											</td>
										</tr>
									</table>
								</div>
