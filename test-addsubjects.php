...
//::START OF 'CONFIGURATION'
	...
	if(isset($_POST['add_subject'])){
		...
		$corequisites = $_POST['corequisites'];
		
		$curr = new CurriculumManager($conn);
		
		//get the curriculum subjects
		$curriculums = $curr->GetCurriculums(1);
		
		if($curr->AddSubject($curriculum, $subject, $semester, $level, $prerequisites, $corequisites) == true){
			echo "success";
			unset($curriculum);
			unset($subject);
			unset($semester);
	...
	if($conn != null){
		...
		$subjects = $hndler->GetSubjectsByCode();
...
<html>
	<body>
						...
						foreach($subjects as $c){
							if(isset($subject)){
								...
								if($subject == $c->subject_id){
									echo "<option value=\"{$c->subject_id}\" selected=\"selected\">{$c->code}\t| {$c->description}</option>";
								} else {
									echo "<option value=\"{$c->subject_id}\" >{$c->code}\t| {$c->description}</option>";
								}
							} else {
								echo "<option value=\"{$c->subject_id}\" >{$c->code}\t| {$c->description}</option>";
							}				
		...
		<?php
			...
			//display subjects per curriculum
			foreach($curriculums as $data){
				echo "<h4>" . $data->school_year . "</h4>";
				$subjects = $cur->GetSubjects($data->curriculum_id);
				...
				foreach($subjects as $subject){
					echo "<li>" . $subject->year_level . " - " . $subject->subject . "</li>";
				}			
				echo "</ol>";
