...
//::END OF 'CONFIGURATION'
	...
	if(isset($_GET['id'])){
		...
		//get subjects array
		$subjects = $hnd->GetSubjectsForEdit($id);
		$subject = $subjects[0];
		...
		$_type = $subject->subject_type;
		$_group = $subject->group;
		$virtual = $subject->virtual;
		$isHalf = $subject->isHalf;

	} else {
		$_SESSION['error'] = array("Unknown Subject.");
								...
								<div class="table_form">
									<table class="form" cellspacing="0">
										...
										<tr>
											...
											<td class="input">:<input type="checkbox" name="virtual" value="1" <?php if($virtual==1){ echo "checked=\"checked\""; } ?>/></td>
										</tr>
										<tr>
											<td class="label">Pay Half Fee</td>
											<td class="input">:<input type="checkbox" name="isHalf" value="1" <?php if($isHalf==1){ echo "checked=\"checked\""; } ?>/></td>
										</tr>
									</table>
									<hr class="form_top"/>
									<table class="form" cellspacing="0">
										<tr class="button">
											<td colspan="2">
												<input type="button" class="button" value="Back" onclick="window.location='schoolplusplus-subjects-view.php?id=<?php echo $id; ?>';"/>
												<input type="submit" class="button" name="subject_save" value="Save" />
											</td>
										</tr>
									</table>
								</div>
