			...
			<div id="body">			
				...
				<div class="content">
					...
					<div class="column" id="column-second">
						<div class="inner">
							<h1>
								<span class="Highlight">College Administration</span> 
							</h1>
							<p class="">Below are the colleges registered in the I-SMS.</p>
							<div id="actions">
								<p class="action">
									<input type="button" value="College Types" onclick="window.location='colleges-type.php'" /> - College types are used to define colleges.
								</p>
								<p class="action">
									<input type="button" value="Add College" onclick="window.location='colleges-add.php'" />
								</p>
							</div>
							<?php
								//##### PASS ERROR IF FOUND
									...
									<?php 
										...
										if(sizeof($colleges) > 0){
											foreach($colleges as $item){
												...
												if($ctr % 2 == 0){
													echo "<tr class=\"even\" onclick=\"window.location='colleges-view.php?id={$item->college_id}';\">";
												} else {
													echo "<tr class=\"odd\" onclick=\"window.location='colleges-view.php?id={$item->college_id}';\">";
												}
													echo "<td>{$ctr}</td>";
													echo "<td>{$item->description}</td>";
													echo "<td>{$item->code}</td>";
													echo "<td>{$item->college_type[0]->description}</td>";
											...
											echo "<td colspan=\"6\">There are no existing college records.</td>";
										}
									?>
