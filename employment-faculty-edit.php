...
//::END OF 'CONFIGURATION'
	...
	//### CHECK IF REQUIRED INFORMATION IS FOUND
	if(isset($_GET['id'])){
		$id = (int) $_GET['id'];
	} else {
		$_SESSION['error'] = array("Faculty Record not found.");
	...
	$employees = $hnd_e->GetAssignedEmployees();
	...
	if(isset($id)){
		if($id > 0){
			$faculties = $hnd->GetFaculties(null, $id);
			if(sizeof($faculties) > 0){
				$faculty = $faculties[0];
				$rank = $faculty->faculty_rank[0]->rank_id;
				$status = $faculty->faculty_status[0]->status_id;
				$college = $faculty->college->college_id;
					...
					<div class="column" id="column-second">
						<div class="inner">
							...
							<form action="employment-faculty-process.php" method="post">
								...
								<input type="hidden" name="faculty" value="<?php echo $employee_id; ?>" />
								<hr class="form_top"/>
									...
									<table class="form" cellspacing="0">
										<tr class="button">
											<td colspan="2">
												...
												<input type="submit" class="button" name="faculty_save" value="Save" />
											</td>
										</tr>
									</table>
								</div>
